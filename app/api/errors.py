...
A model-provider error raised anywhere in a request becomes the same JSON
error body, so endpoints need no try/except of their own. The streaming
endpoint, which can't change its status once it has started, uses
`http_error_for` to word its `error` event the same way.
"""
...
def http_error_for(exc: Exception) -> HTTPError:
    ...
    Listings-API failures never get here - the search tool reports them to
    the model, which tells the user.
    """
